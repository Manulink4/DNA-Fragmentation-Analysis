...
def mw_test(row):
    ...
    if 1 < index < (length-2):
        return mannwhitneyu(list(row)[0:index], list(row)[(index+1):length]).pvalue < 0.22
    else:
        return False
...
def get_significant_cpg_index(df):
    ...
    return df_significant_cpg, significant_cpg_coords
...
def use_mrmr(cpgs, X_train, X_test, y_train, k):
    ...
    selected_features = mrmr_classif(X=X_train, y=y_train, K=k, n_jobs=-1)
...
def use_SelectFromModel(X_train, X_test, y_train, k):
    # clf = ExtraTreesClassifier(n_estimators=k).fit(X_train, y_train)
    clf = LinearSVC(C=0.0001, penalty="l2").fit(X_train, y_train)
